...
class TabloWorkflowService
{
    ...
    /**
     * Create child album and MOVE claimed photos from parent album
     * Called at workflow completion (véglegesítés)
     * Photos are deleted from parent album after copying
     *
     * @deprecated Use finalizeTabloWorkflow() instead
     * @param  User  $user  User completing workflow
     * @param  WorkSession  $childSession  Child work session
     * @param  Album  $parentAlbum  Parent album
     * @param  array<int>  $claimedPhotoIds  Array of photo IDs to move
     * @return array{album: Album, moved: array<int>, conflicts: array<int>}
     */
    public function createChildAlbumAndMovePhotos(
        User $user,
        WorkSession $childSession,
        Album $parentAlbum,
        array $claimedPhotoIds
        ...
        DB::transaction(function () use ($user, $childSession, $parentAlbum, $claimedPhotoIds, &$childAlbum, &$moved, &$conflicts) {
            // 1. Create child album
            $childAlbum = Album::create([
                'title' => "{$parentAlbum->title} - {$user->name}",
                'parent_album_id' => $parentAlbum->id,
            ...
            // 2. Associate with child session
            $childSession->albums()->attach($childAlbum->id);

            // 3. Move photos from parent to child album (FIFO - first to complete wins!)
            foreach ($claimedPhotoIds as $photoId) {
                $photo = Photo::lockForUpdate()
                    ->where('id', $photoId)
                    ->where('album_id', $parentAlbum->id)
                ...
                if (! $photo) {
                    // Photo already moved by another user (FIFO conflict)
                    $conflicts[] = $photoId;
                ...
                // Delete from parent album (first to complete wins!)
                $photo->delete();

                $moved[] = $photoId;

                \Log::info('[TabloCompletion] Photo moved to child album', [
                    'user_id' => $user->id,
                    'photo_id' => $photoId,
                    'parent_album_id' => $parentAlbum->id,
                    'child_album_id' => $childAlbum->id,
